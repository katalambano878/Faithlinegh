...
export default function PrintInventoryPage() {
    ...
    const handleShareWhatsApp = async () => {
        ...
        try {
            ...
            const fileName = `Store-Inventory-${new Date().toISOString().slice(0, 10)}.pdf`;
            const pdfFile = new File([pdfBlob], fileName, { type: 'application/pdf' });

            if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
                await navigator.share({ files: [pdfFile], title: 'Faithlinegh — Inventory Report' });
            } else {
    ...
    return (
      ...
      `}} />
            ...
            <div className="flex items-center justify-between mb-8 border-b-2 border-black pb-4">
                <div>
                    <h1 className="text-3xl font-bold">Faithlinegh</h1>
            ...
            <div className="mt-8 text-center text-sm font-medium border-t-2 border-black pt-4">
                <p>End of Inventory Report &mdash; Generated from Faithlinegh Admin Panel</p>
            </div>
            </div> {/* end contentRef */}
        </div>
    );
