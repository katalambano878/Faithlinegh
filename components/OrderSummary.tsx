    ...
    <div className="bg-white rounded-xl shadow-sm p-6 sticky top-4">
        ...
        {items.map((item) => (
          <div key={`${item.id}-${item.variant || 'novar'}`} className="flex space-x-4">
            <div className="relative w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
              ...
              />
              <div className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center bg-brand-brown text-white text-xs font-bold rounded-full">
                {item.quantity}
              </div>
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-brand-brown text-sm line-clamp-2">{item.name}</h3>
              {item.variant && <p className="text-xs text-gray-500 mt-0.5">{item.variant}</p>}
              <p className="text-brand-brown font-bold mt-1">₵ {item.price.toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-200 mt-4 pt-4">
        <div className="flex justify-between items-center">
          ...
          <span className="text-2xl font-bold text-brand-brown">₵ {total.toFixed(2)}</span>
        </div>
      </div>
