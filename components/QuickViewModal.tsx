  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div 
          className="fixed inset-0 transition-opacity bg-brand-brown/60 backdrop-blur-sm"
          onClick={onClose}
        ></div>
                    ...
                    {images.map((image, index) => (
                      <button
                        ...
                        className={`relative aspect-square bg-gray-100 rounded-lg overflow-hidden border-2 transition-colors ${
                          selectedImage === index ? 'border-brand-brown' : 'border-transparent hover:border-gray-300'
                        }`}
                      >
              ...
              <div className="flex flex-col">
                <div className="mb-2">
                  <Link 
                    ...
                    className="text-sm text-brand-brown hover:text-brand-tan font-medium whitespace-nowrap"
                  >
                    {product.category}
                  </Link>
                </div>

                <h2 className="text-2xl font-bold text-brand-brown mb-3">{product.name}</h2>
                ...
                <div className="flex items-baseline space-x-3 mb-4">
                  <span className="text-3xl font-bold text-brand-brown">₵{product.price.toFixed(2)}</span>
                  {product.originalPrice && (
                    <>
                      <span className="text-lg text-gray-400 line-through">₵{product.originalPrice.toFixed(2)}</span>
                      <span className="px-2 py-1 bg-[#E8DFD4] text-[#5B4436] text-sm font-semibold rounded whitespace-nowrap">
                      ...
                      {product.colors.map((color) => (
                        <button
                          ...
                          className={`px-4 py-2 border-2 rounded-lg font-medium transition-colors whitespace-nowrap ${
                            selectedColor === color
                              ? 'border-brand-brown bg-gray-100 text-brand-brown'
                ...
                <div className="mb-6">
                  <label className="block text-sm font-semibold text-brand-brown mb-2">Quantity</label>
                ...
                <div className="space-y-3 mt-auto">
                  <button
                    ...
                    className={`w-full py-4 rounded-lg font-semibold transition-colors whitespace-nowrap ${
                      addedToCart
                        ...
                        : product.inStock
                        ? 'bg-brand-brown hover:bg-[#47362C] text-white'
                  ...
                  <Link
                    ...
                    className="block w-full py-4 border-2 border-brand-brown text-brand-brown rounded-lg font-semibold text-center hover:bg-gray-50 transition-colors whitespace-nowrap"
                  >
                    View Full Details
                  </Link>
                </div>
