...
const LLM_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Search the website\'s pages and content for information. Use this to answer ANY question about the business, policies, how things work, FAQs, contact info, shipping, returns, payment methods, account management, checkout process, blog content, or anything else about the store. This searches all public pages of the website. ALWAYS use this tool when a customer asks about the business, policies, processes, or anything non-product related.',
      parameters: {
        type: 'object',
        properties: {
      ...
      parameters: {
        ...
        properties: {
          ...
          delivery_method: {
            ...
            description: 'Delivery method. Standard: ₵20, Express: ₵40, Pickup: Free',
          },
          payment_method: {
            type: 'string',
            enum: ['moolre', 'cod'],
...
function buildSystemPrompt(profile: ChatCustomerProfile | null, pagePath?: string): string {
  ...
  let prompt = `You are the AI shopping assistant for Faithlinegh — a Ghanaian fashion store offering quality, affordable bags, basics and dresses for women. We curate stylish everyday pieces and offer nationwide delivery across Ghana. Today is ${now}.
...
- Always quote prices in ₵ (GHS, Ghana Cedis).
...
5. When the customer provides information (like an email address), ACKNOWLEDGE it specifically (e.g. "Got it, using the email you provided...") and proceed with the action immediately.
6. Maintain context throughout the entire conversation. If the customer said they want a password reset 3 messages ago, you still know that's what they want. (When acknowledging info, use the value the customer actually provided, e.g. "Got it, using customer@example.com...".)
...
- Delivery: Nationwide delivery across Ghana (timing depends on destination)
- Returns: Within 30 days of delivery, unused items in original packaging
- Payment: mobile money, card payment, Cash on Delivery (Accra only)
