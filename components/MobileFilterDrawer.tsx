      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[90vh] flex flex-col">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-4 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-brand-brown">Filters</h2>
            <button
              onClick={onClose}
          ...
          {getActiveFiltersCount() > 0 && (
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                onClick={clearAll}
                className="text-sm text-brand-brown font-medium whitespace-nowrap"
              >
                Clear All
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div>
            <h3 className="text-sm font-semibold text-brand-brown mb-3">Category</h3>
              ...
              {categories.map((category) => (
                <label key={category} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-brand-brown border-gray-300 rounded focus:ring-gray-600"
                  />
          ...
          <div>
            <h3 className="text-sm font-semibold text-brand-brown mb-3">Price Range</h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-brand-brown mb-3">Size</h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-brand-brown mb-3">Color</h3>
                ...
                >
                  {selectedColors.includes(color.name) && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <i className={`ri-check-line text-lg ${color.hex === '#FFFFFF' ? 'text-brand-brown' : 'text-white'}`}></i>
                    </div>
                  )}
                </button>
              ))}
          ...
          <div>
            <h3 className="text-sm font-semibold text-brand-brown mb-3">Customer Rating</h3>
            <div className="space-y-2">
              {[4, 3, 2, 1].map((rating) => (
                <label key={rating} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-brand-brown border-gray-300 focus:ring-gray-600"
                  />
